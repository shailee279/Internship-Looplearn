...
def detect(opt):
    ...
    for root, dirs, files in os.walk(opt.images, topdown=True):
        ...
        for name in sorted(files):
            ...
            pred = faceCascade.detectMultiScale(
                np.array(img), scaleFactor=scale, minNeighbors=min_n,
            ...
            pred = np.append(pred, np.ones((np.shape(pred)[0], 2)), axis=1) # add obj conf as 1
